...
def patient_status(request, pid):
    ...
    progerss_percent = calculate_persentage(
        clickedpatient.treatment_started_date, clickedpatient.treatment_end_date
    )
    ...
    context = dict(
        p_str=p_str,
        pid=pid,
        clickedpatient=Patient.objects.filter(
            id=pid
        ),  # 여기 좀 손봐야됨 (dashboard.html 에서 무지성 for 문)
...
def patient_inspection(request, pid):
    ...
    if start_date and end_date:
        sputum = Sputum_Inspection.objects.filter(
            patient_set=pid,
            deleted=False,
            insp_date__gte=start_date,
            insp_date__lte=end_date,
        ).order_by("-id")
    else:
        sputum = Sputum_Inspection.objects.filter(
            patient_set=pid, deleted=False
        ).order_by("-id")
...
def inspection_delete(request, sputum_id):
    if request.method == "DELETE":
        ...
        return JsonResponse(
            {"message": "Sputum result ${sputum_id} deleted successfully."}
        )
    else:
...
def transform_likert(symptom_list: dict, symptom_name: str) -> dict:
    ...
    if symptom_name in symptom_list:
        return {
            "frequency": make_likert(symptom_list[symptom_name][0])
            if symptom_list[symptom_name]
            else 0,
            "intensity": make_likert(symptom_list[symptom_name][1])
            if symptom_list[symptom_name]
            else 0,
...
def get_likert_score_by_symptom(symptom_db: str, symptom_name: str) -> dict:
    ...
    if symptom_name:
        for i in symptom_db:
            rst[get_date_text_simple(i.date, True)] = transform_likert(
                i.get_symptom_severity_list(), symptom_name
            )
    return rst
