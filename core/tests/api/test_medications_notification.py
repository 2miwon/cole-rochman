...
class PastMedicationEntranceTest(APITestCase):
    ...
    def test_success(self):
        ...
        p.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('지난번 복약 이후에 몸에 이상 반응은 없었나요?', message_in_response(response))
...
class PastMedicationSuccessTest(APITestCase):
    ...
    def test_success(self):
        ...
        p.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(p.medication_results.last().get_status(), MedicationResult.STATUS.SUCCESS)
...
class PastMedicationSideEffectTest(APITestCase):
    ...
    def test_success(self):
        ...
        p.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(p.medication_results.last().get_status(), MedicationResult.STATUS.SIDE_EFFECT)
        self.assertEqual(p.medication_results.last().medication_time_num, 1)
