...
set _WRAPPER_EXE=%_REALPATH%wrapper/%_WRAPPER_BASE%-windows-x86-32.exe
if exist "%_WRAPPER_EXE%" goto conf
set _WRAPPER_EXE=%_REALPATH%wrapper/%_WRAPPER_BASE%-windows-x86-64.exe
if exist "%_WRAPPER_EXE%" goto conf
set _WRAPPER_EXE=%_REALPATH%wrapper/%_WRAPPER_BASE%.exe
if exist "%_WRAPPER_EXE%" goto conf
echo Unable to locate a Wrapper executable using any of the following names:
echo %_REALPATH%wrapper/%_WRAPPER_BASE%-windows-x86-32.exe
echo %_REALPATH%wrapper/%_WRAPPER_BASE%-windows-x86-64.exe
echo %_REALPATH%wrapper/%_WRAPPER_BASE%.exe
pause
goto :eof
...
set _WRAPPER_CONF="%_REALPATH%wrapper/wrapper.conf"
