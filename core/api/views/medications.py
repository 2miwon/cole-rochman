...
class PatientMedicationEntrance(KakaoResponseAPI):
    ...
    def post(self, request, format="json", *args, **kwargs):
        ...
        try:
            ...
            noti_time_table = self.get_notification_time_by_kakao_id()
        except Http404:
        ...
        if len(noti_time_table):
            response.add_simple_text(
                "안녕하세요 콜로크만입니다.\n지난 번, 복약 관리를 설정한 적이 있습니다.\n다시 설정할까요?"
            )
            response.add_simple_text(len(self.get_notification_time_by_kakao_id()))
            response.set_quick_replies_yes_or_no(
                ...
                message_text_for_yes="네, 설정할게요!",
                message_text_for_no="아니요, 괜찮아요!",
            )
            response.add_context(
                name="복약관리재시작",
                # params={"daily_medication_count": patient.daily_medication_count},
                params={"daily_medication_count": len(noti_time_table)},
            )
        else:
...
class PatientMedicationNotiTimeQuestion(KakaoResponseAPI):
    ...
    def post(self, request, format="json", *args, **kwargs):
        ...
        if (
            not patient.need_medication_noti_time_set()
            and len(patient.medication_noti_time_list()) != 0
        ):
            time_list = ",".join(
                [x.strftime("%H시 %M분") for x in patient.medication_noti_time_list()]
            )
            ...
            response.set_quick_replies_yes_or_no(
                block_id_for_yes="5db30f398192ac000115f9a0"
            )  # (블록) 02 치료 관리 재설정_복약횟수 확인
        else:
            if self.request.query_params.get("restart") == "true":
                response.set_quick_replies_yes_or_no(
                    block_id_for_yes="5db3129e8192ac000115f9a8"
                )  # (블록) 05 치료 관리 재설정_복약 알림 시간대
            else:
                response.set_quick_replies_yes_or_no(
                    block_id_for_yes="5da68fb1ffa7480001db0361"
                )  # (블록) 04 치료 관리 설정_복약 알림 시간대
