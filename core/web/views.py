...
def patient_status(request,pid):
    ...
    context = dict(
        ...
        b=MedicationResult.objects.filter(patient__id__contains=pid,reserved_at__gte=cal_start_end_day(d,1), reserved_at__lte=cal_start_end_day(d,7)),
    ...
    if (clickedpatient.daily_medication_count >= 2):
        daily_hour_list.append(str(clickedpatient.medication_noti_time_2.hour) + ":" + str(clickedpatient.medication_noti_time_2.minute))
    if (clickedpatient.daily_medication_count >= 3):
        daily_hour_list.append(str(clickedpatient.medication_noti_time_3.hour) + ":" + str(clickedpatient.medication_noti_time_3.minute))
    if (clickedpatient.daily_medication_count >= 4):
        daily_hour_list.append(str(clickedpatient.medication_noti_time_4.hour) + ":" + str(clickedpatient.medication_noti_time_4.minute))
    if (clickedpatient.daily_medication_count >= 5):
        daily_hour_list.append(str(clickedpatient.medication_noti_time_5.hour) + ":" + str(clickedpatient.medication_noti_time_5.minute))
